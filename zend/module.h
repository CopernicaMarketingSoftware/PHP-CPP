...
namespace Php
{
    ...
    public:
        ...
        Module(const char *module, bool persistent)
        {
            ...
            if (persistent && !_persistent.contains(_handle))
                _persistent.add(module);
        ...
        bool valid() const
        {
            ...
            if (_entry->zend_api != ZEND_MODULE_API_NO)
                return false;
        ...
        bool start()
        {
            ...
            // startup the module
            if (zend_startup_module_ex(entry) == FAILURE)
                return false;
            ...
            if (entry->request_startup_func)
            {
                ...
                if (entry->request_startup_func(MODULE_TEMPORARY, entry->module_number) == FAILURE)
                    return false;
            }
