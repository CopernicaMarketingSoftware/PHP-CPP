...
#define DOC_COMMENT info.user.doc_comment
#else
#define DOC_COMMENT doc_comment
#endif
 ...
 */
    ...
    zend_object_handlers *ClassImpl::objectHandlers()
    {
        ...
        if (!_base->clonable())
            ...
            _handlers.clone_obj = &ClassImpl::cloneObject;
...
#else
            if (!std_object_handlers.compare)
                return 1;
 ...
 */
...
#endif
    {
        ...
        try
        {
            ...
            {
            case Type::Numeric:
                result = meta->callToInteger(object);
                break;
            case Type::Float:
                result = meta->callToFloat(object);
                break;
            case Type::Bool:
                result = meta->callToBool(object);
                break;
            case Type::String:
                result = meta->callToString(object);
                break;
            default:
                throw NotImplemented();
                break;
    ...
    zend_object *ClassImpl::cloneObject(ZEND_OBJECT_OR_ZVAL val)
    {
        ...
        if (!cpp)
            zend_error(E_ERROR, "Unable to clone %s", entry->name->val);
 ...
 */
    ...
    int ClassImpl::hasDimension(ZEND_OBJECT_OR_ZVAL object, zval *member, int check_empty)
    {
        ...
        if (arrayaccess)
        {
            ...
            try
            {
                ...
                if (!arrayaccess->offsetExists(member))
                    return false;
    ...
    int ClassImpl::hasProperty(ZEND_OBJECT_OR_ZVAL object, ZEND_STRING_OR_ZVAL name, int has_set_exists, void **cache_slot)
    {
        ...
        try
        {
            ...
            if (impl->_properties.find(key) != impl->_properties.end())
                return true;

            // call the C++ object
            if (!meta->callIsset(base, key))
                return false;
    ...
    void ClassImpl::unsetProperty(ZEND_OBJECT_OR_ZVAL object, ZEND_STRING_OR_ZVAL member, void **cache_slot)
    {
        ...
        try
        {
            ...
            if (iter == impl->_properties.end())
                impl->_base->callUnset(ObjectImpl::find(object)->object(), member);
    ...
    void ClassImpl::destructObject(zend_object *object)
    {
        ...
        try
        {
            ...
            if (obj->object())
                impl->_base->callDestruct(obj->object());
        }
        catch (const NotImplemented &exception)
        {
    ...
    {
        // find the method
        auto result = std::find_if(_methods.begin(), _methods.end(), [name](std::shared_ptr<Method> method)
    ...
    const struct _zend_function_entry *ClassImpl::entries()
    {
        ...
        if (_base->countable() && !hasMethod("count"))
            entrycount += 1;
        ...
        if (_base->serializable())
        {
            ...
            if (!hasMethod("serialize"))
                entrycount += 1;
            if (!hasMethod("unserialize"))
                entrycount += 1;
        }
        ...
        if (_base->countable())
        {
            ...
            if (!hasMethod("count"))
                count.initialize(&_entries[i++], _name);
        }
        ...
        if (_base->serializable())
        {
            ...
            // register the serialize and unserialize method in case this was not yet done in PHP user space
            if (!hasMethod("serialize"))
                serialize.initialize(&_entries[i++], _name);
            if (!hasMethod("unserialize"))
                unserialize.initialize(&_entries[i++], _name);
        }
        ...
        if (_base->traversable())
        {
            ...
            // register the serialize and unserialize method in case this was not yet done in PHP user space
            if (!hasMethod("getIterator"))
                getIterator.initialize(&_entries[i++], _name);
        }
    ...
    zend_class_entry *ClassImpl::initialize(ClassBase *base, const std::string &prefix)
    {
        ...
        for (auto &interface : _interfaces)
        {
            ...
            // otherwise report an error
                ...
                std::cerr << "Derived class " << name() << " is initialized before base class " << interface->name() << ": interface is ignored" << std::endl;
        }
