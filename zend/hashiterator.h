...
public:
     ...
     *  @param  tsrm_ls
     */
    HashIterator(HashTable *hashtable, bool first, bool is_array = false) : _table(hashtable), is_array(is_array)
    {
     ...
     *  @param  tsrm_ls
     */
    HashIterator(const HashIterator &that TSRMLS_DC, bool is_array = false) :
        _table(that._table), _position(that._position), is_array(is_array)
    {
...
private:
    ...
    bool read()
    {
        ...
        return is_array || !_current.first.isString() || _current.first.rawValue()[0];
    }
